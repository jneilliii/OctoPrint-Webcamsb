...
class WebcamSBPlugin(octoprint.plugin.StartupPlugin,
	...
	def get_settings_defaults(self):
		return dict(url="",url1="",url2="",url3="",expand=1,defaultCam=1,urls=[])
	...
	def get_template_configs(self):
		return [
			dict(type="sidebar", name="Stream", template="webcamsb_sidebar.jinja2", icon="camera", custom_bindings=True),
			dict(type="settings", name="Sidebar Webcam", template="webcamsb_settings.jinja2", custom_bindings=True),
			dict(type="generic", name="Full Screen", template="webcamsb.jinja2", custom_bindings=False)
		]
